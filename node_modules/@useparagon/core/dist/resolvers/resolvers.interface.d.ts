...
export type ConnectCredentialSource = {
    ...
    type: 'CONNECT_CREDENTIAL_FIELD';
    fieldType: 'SAVED_CREDENTIAL_DATA';
    dataKey: string;
} | {
    type: 'CONNECT_CREDENTIAL_FIELD';
...
export type BYOWizardOauthAppSource = {
    type: 'BYO_OAUTH_APP_CREDENTIAL';
    fieldType: 'CREDENTIAL_VALUES';
    dataKey: string;
};
export type BYOWizardOauthAuthRequestSource = {
    type: 'BYO_OAUTH_APP_CREDENTIAL';
    fieldType: 'REQUEST_PARAMS';
    requestName: 'authRequest' | 'tokenRequest';
    dataKeys: string[];
};
export type BYOWizardOauthSource = BYOWizardOauthAppSource | BYOWizardOauthAuthRequestSource;
...
export type WorkflowExecutionSource = {
    type: 'WORKFLOW_EXECUTION_FIELD';
    fieldType: 'EXECUTION_ID';
} | {
    type: 'WORKFLOW_EXECUTION_FIELD';
    fieldType: 'PROJECT_ID';
} | {
    type: 'WORKFLOW_EXECUTION_FIELD';
    fieldType: 'WORKFLOW_ID';
};
export type TokenizedValue<T extends DataType = DataType> = ValueSource<T> | VariableSource<T> | SecretSource | ConnectCredentialSource | PersonaMetadataSource | SameStepReferenceSource<T> | CustomWebhookTriggerSource | BYOWizardOauthSource | UUIDSource | WorkflowExecutionSource;
export type TokenizedSource<T extends DataType = DataType> = {
    dataType?: T;
    type: 'TOKENIZED';
    parts: TokenizedValue[];
};
export type Source<T extends DataType = DataType> = ValueSource<T> | VariableSource<T> | TokenizedSource<T> | ConditionSource | SecretSource | PersonaMetadataSource | ConnectCredentialSource | SameStepReferenceSource<T> | CustomWebhookTriggerSource | BYOWizardOauthSource | UUIDSource | WorkflowExecutionSource;
