...
export type NonGraphQLRequestBodyType = RequestBodyType.JSON | RequestBodyType.FORM_DATA | RequestBodyType.X_WWW_FORM_URLENCODED | RequestBodyType.XML | RequestBodyType.RAW;
export type BaseRequestParams = Pick<IRequestStepParameters, 'httpMethod' | 'ignoreFailure' | 'retryOnFailure' | 'url' | 'params' | 'headers' | 'authorization'>;
export type IRequestStepParameters = IBaseStepParameters & IStepIgnoreFailureParameters & IStepRetryParameters & {
    ...
    bodyType?: NonGraphQLRequestBodyType;
    body: KeyedSource[];
    rawBody?: TokenizedSource<DataType.ANY>;
} | {
    bodyType: RequestBodyType.GRAPHQL;
    query: TokenizedSource<DataType.STRING>;
    variables?: TokenizedSource<DataType.STRING>;
});
